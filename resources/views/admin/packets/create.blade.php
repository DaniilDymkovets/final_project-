...
@section('content')
    ...
    <div class="page-header">
        <h2>{{ trans('admins.paсket_create_page') }}</h2>
    </div>
    @if (session('error'))
    ...
    <form class="form-horizontal" method="POST" action="{{ route('packets.store')}}" accept-charset="UTF-8">
        {{ csrf_field() }}
        ...
        <div class="form-group{{ $errors->has('bonus') ? ' has-error' : '' }}">
            ...
            <div class="col-md-6">
                <input id="bonus" type="text" class="form-control" name="bonus" 
                       value="{{ old('bonus')?old('bonus'):0 }}" required>
                @if ($errors->has('bonus'))
                    <span class="help-block">
